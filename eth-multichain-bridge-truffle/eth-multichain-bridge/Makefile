...
console: start run-console

test: start run-test

run-console: check-deps ganache-conn
	truffle console

run-test: check-deps ganache-conn
	truffle test

start: ganache eth-bridge localtunnel
...
eth-bridge: ganache-conn
	ethereum-bridge -H localhost:$(STD_DEV_PORT) -a 0 &> bridge.log
